...
export function findInsideJokes(state, chatId, text, limit = 20) {
    ...
    return Object.values(state?.banks || {})
        ...
        .flatMap(bank => {
            const annotated = (Array.isArray(bank.jokes) ? bank.jokes : []).map(joke => ({
                ...joke,
                bankName: bank.name,
                matchedKeywords: joke.keywords.filter(keyword => normalizedMessage.includes(normalizeInsideJokeText(keyword)))
            }));
            if (!annotated.some(joke => joke.matchedKeywords.length > 0))
                return [];
            return annotated.map(joke => ({
                ...joke,
                relatedContext: joke.matchedKeywords.length === 0
            }));
        })
        .sort((left, right) => {
            const leftLength = left.matchedKeywords.length ? Math.max(...left.matchedKeywords.map(keyword => keyword.length)) : 0;
            const rightLength = right.matchedKeywords.length ? Math.max(...right.matchedKeywords.map(keyword => keyword.length)) : 0;
