...
export async function renderLastFmCard(track, labels = {}) {
    ...
    const base = cover
        ? await sharp(cover).resize(600, 250, { fit: 'cover' }).blur(18).modulate({ brightness: 0.82, saturation: 0.95 }).jpeg().toBuffer()
    ...
    const coverLayer = cover
        ...
        : null;
    const artist = truncate(track.artist, 30);
    const name = truncate(track.name, 26);
    const username = truncate(`@${track.username}`, 24);
    ...
    const svg = `<svg width="600" height="250" xmlns="http://www.w3.org/2000/svg">
      <rect width="600" height="250" fill="#000" fill-opacity=".20"/>
      <rect x="16" y="16" width="218" height="218" rx="8" fill="#000" fill-opacity=".16"/>
      <style>.title{font:700 20px Arial,sans-serif;fill:#fff}.artist{font:18px Arial,sans-serif;fill:#eee}.small{font:14px Arial,sans-serif;fill:#eee}.muted{font:14px Arial,sans-serif;fill:#bbb}</style>
