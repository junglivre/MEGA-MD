...
export function getMessageSenderJid(message) {
    ...
    const participantIds = [key.participant, key.participantAlt].filter(Boolean);
    const lid = participantIds.find(value => String(value).includes('@lid'));
    const pn = participantIds.find(value => value !== lid && !String(value).includes('@lid'));
    if (lid && pn)
...
/**
 * Return every known representation of the sender, including a remembered
 * LID/PN mapping. This is intentionally used for short-lived conversational
 * state (such as the inside-joke wizard), where the same person may arrive
 * with a different preferred JID on the next message.
 */
export function getMessageIdentityAlternatives(message) {
    const alternatives = getMessageJidAlternatives(message);
    for (const jid of [...alternatives]) {
        const alternate = getAlternateJid(jid);
        if (alternate)
