...
const store = {
    ...
    bind(ev) {
        ev.on('messages.upsert', async ({ messages }) => {
            for (const msg of messages) {
                ...
                // Keep a recent in-memory window even when the durable backend
                // is SQL/Mongo. Features such as the quote command need the
                // surrounding messages, not only a single message by ID.
    ...
    /**
     * Per-user settings. The file backend stores these as a map keyed by JID;
     * SQL/document backends use their existing chat_id/key abstraction.
     */
    async saveUserSetting(userId, key, value) {
        if (backend === 'memory') {
            ...
            try {
                ...
                console.error(`[STORE] Failed to read user setting ${key}:`, e.message);
            }
            data[userId] = value;
            try {
                fs.writeFileSync(filePath, JSON.stringify(data, null, 2));
                ...
                console.error(`[STORE] Failed to save user setting ${key}:`, e.message);
            }
            return;
        }
        try {
            await adapters[backend].saveSetting(userId, key, value);
        }
        catch (e) {
            console.error(`[STORE] Failed to save user setting ${key}:`, e.message);
        }
    ...
    async getUserSetting(userId, key) {
        if (backend === 'memory') {
            ...
            try {
                ...
                console.error(`[STORE] Failed to get user setting ${key}:`, e.message);
                return null;
            }
        }
        try {
            return await adapters[backend].getSetting(userId, key);
        }
        catch (e) {
            console.error(`[STORE] Failed to get user setting ${key}:`, e.message);
            return null;
