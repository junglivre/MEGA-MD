...
export function translate(language, key, vars = {}) {
    ...
    const resolved = replaceCommandPrefix(text, prefix);
    return resolved.replace(/\{(\w+)\}/g, (_, name) => ({ prefix, ...vars })[name] ?? `{${name}}`);
}

export function getConfiguredPrefix() {
    return config.prefixes?.find(value => String(value).trim()) || config.prefix || '.';
}

export function replaceCommandPrefix(text, prefix = getConfiguredPrefix()) {
    const commandPrefixPattern = /(^|[\s`])\.(?=[a-z][a-z0-9_-]*(?:\s|[`<[]|$))/gi;
    return String(text || '').replace(commandPrefixPattern, (_match, leading) => `${leading}${prefix}`);
}
