...
import { getMessageSenderJid } from './jid.js';
...
function strictOwnerOnlyMessage(command, t) {
    const notice = command.strictOwnerOnlyNotice
        ? t(command.strictOwnerOnlyNotice)
        : t('sudoManageNote');
    return `ℹ️ *${t('ownerOnlyCommand')}*\n\n_${notice}_`;
}
...
async function handleMessages(sock, messageUpdate) {
    try {
                        ...
                        if (foundCommand.strictOwnerOnly) {
                            ...
                            if (!message.key.fromMe && !(await isOwnerOnlyInContext(senderId, sock, chatId))) {
                                return await sock.sendMessage(chatId, {
                                    text: strictOwnerOnlyMessage(foundCommand, t),
                                    ...channelInfo
                                }, { quoted: message });
                            }
                        }
        ...
        if (command.strictOwnerOnly) {
            ...
            if (!message.key.fromMe && !(await isOwnerOnlyInContext(senderId, sock, chatId))) {
                return await sock.sendMessage(chatId, {
                    text: strictOwnerOnlyMessage(command, createTranslator()),
                    ...channelInfo
                }, { quoted: message });
            }
        }
