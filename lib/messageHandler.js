...
import { handleInsideJokesWizard } from '../plugins/insidejokes.js';
...
import { createTranslator, getUserLanguage } from './i18n.js';
import { getMessageIdentityAlternatives, getMessageSenderJid } from './jid.js';
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // Store pushName under every ID supplied by Baileys (preferred LID and PN alt).
        if (message.pushName && sock.store?.contacts) {
            for (const pid of [message.key.participant, message.key.participantAlt, message.key.remoteJid, message.key.remoteJidAlt].filter(Boolean)) {
        ...
        // Keep the preferred LID as the canonical identity. PN is only an alt.
        const senderId = getMessageSenderJid(message);
                ...
                if (stickers[hash]) {
                    ...
                    if (foundCommand) {
                        ...
                        if (foundCommand.strictOwnerOnly) {
                            const { isOwnerOnlyInContext } = await import('./isOwner.js');
                            if (!message.key.fromMe && !(await isOwnerOnlyInContext(senderId, sock, chatId))) {
                                return await sock.sendMessage(chatId, {
                                    text: `ℹ️ *${t('ownerOnlyCommand')}*\n\n_${t('sudoManageNote')}_`,
                                    ...channelInfo
                                }, { quoted: message });
                            }
                        }
                        if (foundCommand.ownerOnly && !message.key.fromMe && !senderIsOwnerOrSudo) {
                            return await sock.sendMessage(chatId, {
                                text: `ℹ️ *${t('ownerOrSudoOnly')}*`,
                                ...channelInfo
                            }, { quoted: message });
                        }
                        if (foundCommand.groupOnly && !isGroup) {
                            return await sock.sendMessage(chatId, {
                                text: `ℹ️ *${t('groupOnlyCommand')}*`,
                                ...channelInfo
                            }, { quoted: message });
                        }
                        ...
                        try {
                            ...
                            await sock.sendMessage(chatId, {
                                text: `❌ ${t('stickerCommandError', { error: error.message })}`,
                                ...channelInfo
                            }, { quoted: message });
                        }
                    }
        ...
        const senderIsOwnerOrSudo = await isOwnerOrSudo(senderId, sock, chatId);
        const insideJokesWizardHandled = await handleInsideJokesWizard(sock, message, {
            chatId,
            senderId,
            senderAlternatives: getMessageIdentityAlternatives(message),
            rawText,
            config,
            senderIsOwnerOrSudo
        });
        if (insideJokesWizardHandled)
            return;
        ...
        if (command.adminOnly && isGroup) {
            ...
            if (!isBotAdmin) {
                return await sock.sendMessage(chatId, {
                    text: `ℹ️ *${(createTranslator(await getUserLanguage(senderId)))('adminRequired')}*`,
                    ...channelInfo
                }, { quoted: message });
            }
            if (!isSenderAdmin && !message.key.fromMe && !senderIsOwnerOrSudo) {
                return await sock.sendMessage(chatId, {
                    text: `ℹ️ *${(createTranslator(await getUserLanguage(senderId)))('adminOnly')}*`,
                    ...channelInfo
                }, { quoted: message });
            }
        }
        ...
        try {
            ...
            console.error(error.stack);
            await sock.sendMessage(chatId, {
                text: `❌ ${t('errorCommand', { error: error.message })}`,
                ...channelInfo
            }, { quoted: message });
            const errorLog = {
        ...
        if (chatId) {
            try {
                const failMsg = messageUpdate.messages?.[0];
                const failLanguage = await getUserLanguage(failMsg ? getMessageSenderJid(failMsg) : null);
                await sock.sendMessage(chatId, {
                    text: `ℹ️ *${createTranslator(failLanguage)('failedProcessMessage')}*`,
                    ...channelInfo
                });
            }
            catch (e) {
