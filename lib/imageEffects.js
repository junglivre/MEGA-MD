...
function petPetHand(top) {
    return Buffer.from(
        '<svg xmlns="http://www.w3.org/2000/svg" width="128" height="128">'
        + `<g transform="translate(0 ${top})">`
        + '<path d="M-9 13 C10 2 32 0 51 5 C67 1 85 4 96 11 C108 13 120 19 137 30 '
        + 'L132 51 C111 47 94 42 77 39 C57 35 39 37 21 44 C9 48 -3 45 -10 38 Z" '
        + 'fill="#e6bea0" stroke="#6d4a3a" stroke-width="3" stroke-linejoin="round"/>'
        + '<path d="M46 6 C57 3 67 4 75 9 M70 7 C81 5 91 8 98 14 M94 13 C104 13 113 17 120 23" '
        + 'fill="none" stroke="#9b6a54" stroke-width="2.2" stroke-linecap="round"/>'
        + '<path d="M-8 10 C6 5 16 4 27 6 L22 43 C9 49 -2 46 -10 39 Z" fill="#e8e8e8" '
        + 'stroke="#6b6b6b" stroke-width="3"/>'
        + '</g></svg>'
    );
}

export async function createPetPetGif(input) {
    const size = 128;
    const source = await sharp(input, { limitInputPixels: MAX_INPUT_PIXELS })
        ...
        .toBuffer();
    const poses = [
        { left: 13, top: 25, width: 102, height: 100, handTop: -8 },
        { left: 10, top: 30, width: 108, height: 95, handTop: -2 },
        { left: 7, top: 38, width: 114, height: 87, handTop: 7 },
        { left: 10, top: 31, width: 108, height: 94, handTop: 0 },
        { left: 13, top: 25, width: 102, height: 100, handTop: -8 }
    ];
    const frames = [];
    for (const pose of poses) {
        ...
        frames.push(await sharp({
            ...
            { input: petPetHand(pose.handTop), left: 0, top: 0 }
        ]).png().toBuffer());
    }
    const strip = sharp({
        create: {
    ...
    return strip.gif({ loop: 0, delay: poses.map(() => 80), effort: 3, colours: 128 }).toBuffer();
}
