...
export async function createPrideOverlayImage(input, opacity = 0.22) {
    const { data, info } = await normalizedImage(input);
    const safeOpacity = Math.max(0.08, Math.min(0.35, Number(opacity) || 0.22));
...
async function createPlacedTemplateImage(input, templateInput, placement) {
    ...
    let photo = await sharp(input, { limitInputPixels: MAX_INPUT_PIXELS })
        .rotate()
        .resize(width, height, {
        ...
        .png()
        .toBuffer();
    if (placement.radius || placement.polygon || placement.feather) {
        const feather = Math.max(0, Math.min(
            Number(placement.feather) || 0,
            Math.floor(Math.min(width, height) / 3)
        ));
        const definitions = feather
            ? `<defs><filter id="soft-edge"><feGaussianBlur stdDeviation="${feather}"/></filter></defs>`
            : '';
        const shape = placement.polygon
            ? `<polygon points="${placement.polygon.map(point => point.join(',')).join(' ')}" fill="white"/>`
            : `<rect x="${feather}" y="${feather}" width="${width - feather * 2}" height="${height - feather * 2}" `
                + `rx="${placement.radius || feather}" fill="white"${feather ? ' filter="url(#soft-edge)"' : ''}/>`;
...
export async function createEdnaldoTvImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 269,
        top: 22,
        width: 142,
...
export async function createBolsonaroTvImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 109,
        top: 12,
        width: 273,
...
export async function createStudiopolisTvImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 153,
        top: 68,
        width: 186,
...
export async function createBolsonaroTv2Image(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 213,
        top: 36,
        width: 222,
        height: 130,
        polygon: [[2, 0], [221, 5], [217, 129], [0, 125]],
        fit: 'fill'
    });
}

export async function createAtaImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 105,
        top: 0,
        width: 195,
        height: 177,
        polygon: [
            [0, 0], [195, 0], [195, 177], [59, 151], [55, 145], [49, 138], [43, 130],
            [38, 120], [37, 110], [38, 98], [39, 83], [39, 68], [36, 55], [32, 44],
            [27, 35], [24, 27], [21, 20], [15, 15], [8, 12], [3, 9]
        ],
        fit: 'fill'
    });
}

export async function createRipTvImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 557,
        top: 67,
        width: 155,
        height: 109,
        polygon: [
            [7, 0], [155, 2], [148, 108], [0, 101], [2, 84], [12, 80],
            [13, 73], [11, 64], [8, 54], [8, 42]
        ],
        fit: 'fill'
    });
}

export async function createGodImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 1,
        top: 1,
        width: 87,
...
export async function createBobFireImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 27,
        top: 37,
        width: 103,
        height: 135,
        polygon: [
            [0, 0], [39, 0], [41, 4], [46, 7], [51, 5], [55, 8], [61, 8],
            [66, 5], [70, 4], [74, 7], [79, 7], [83, 2], [103, 2], [103, 135], [0, 135]
        ],
        fit: 'fill'
    });
}

export async function createBolsoFrameImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 303,
        top: 36,
        width: 94,
        height: 123,
        polygon: [[8, 0], [94, 10], [86, 122], [0, 115]],
        fit: 'fill'
    });
}

export async function createPassingPaperImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 179,
        top: 215,
        width: 135,
        height: 104,
        polygon: [
            [41, 0], [135, 29], [94, 104], [80, 104], [77, 101], [70, 93], [64, 88],
            [57, 86], [52, 91], [47, 99], [45, 104], [39, 92], [36, 86], [30, 81],
            [22, 77], [14, 74], [8, 70], [0, 63]
        ],
        fit: 'fill'
    });
}

export async function createBriggsCoverImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 221,
        top: 67,
        width: 164,
        height: 194,
        polygon: [
            [22, 0], [164, 21], [155, 151], [146, 157], [139, 166],
            [133, 178], [127, 192], [0, 187]
        ],
        fit: 'fill'
    });
}

export async function createBuckShirtImage(input, templateInput) {
    const cleanupPlacements = [
        { left: 199, top: 106, width: 127, height: 133, feather: 5, fit: 'fill' },
        { left: 43, top: 91, width: 51, height: 45, feather: 2, fit: 'fill' },
        { left: 53, top: 205, width: 37, height: 36, feather: 2, fit: 'fill' }
    ];
    const placements = [
        { left: 205, top: 112, width: 115, height: 121, feather: 7, fit: 'fill' },
        { left: 46, top: 94, width: 45, height: 39, feather: 3, fit: 'fill' },
        { left: 56, top: 208, width: 31, height: 30, feather: 2, fit: 'fill' }
    ];
    const white = await sharp({
        create: { width: 1, height: 1, channels: 4, background: '#ffffff' }
    }).png().toBuffer();
    let result = templateInput;
    for (const placement of cleanupPlacements)
        result = await createPlacedTemplateImage(white, result, placement);
    for (const placement of placements)
        result = await createPlacedTemplateImage(input, result, placement);
    return result;
}

export async function createPepeDreamImage(input, templateInput) {
    const white = await sharp({
        create: { width: 1, height: 1, channels: 4, background: '#ffffff' }
    }).png().toBuffer();
    const cleanTemplate = await createPlacedTemplateImage(white, templateInput, {
        left: 96,
        top: 7,
        width: 70,
        height: 96,
    ...
    return createPlacedTemplateImage(input, cleanTemplate, {
        left: 99,
        top: 10,
        width: 65,
        height: 91,
...
export async function createRomeroBrittoImage(input, templateInput) {
    return createPlacedTemplateImage(input, templateInput, {
        left: 25,
        top: 20,
        width: 178,
        height: 269,
        polygon: [
            [0, 0], [177, 14], [174, 37], [168, 43], [166, 63], [163, 76],
            [153, 90], [148, 107], [140, 123], [136, 130], [132, 129], [126, 133],
            [123, 139], [124, 146], [129, 151], [138, 157], [145, 164], [150, 175],
            [154, 194], [153, 211], [147, 225], [137, 235], [117, 243], [91, 253],
            [65, 267], [19, 269]
        ],
        fit: 'fill'
    });
...
export async function createWolverineFrameImage(input, templateInput) {
    const result = await createPlacedTemplateImage(input, templateInput, {
        left: 54,
        top: 160,
        width: 108,
        height: 136,
        polygon: [[2, 0], [101, 0], [108, 132], [10, 136]],
        fit: 'fill'
    });
    const template = await sharp(templateInput, { limitInputPixels: MAX_INPUT_PIXELS })
        .ensureAlpha()
        .png()
        .toBuffer({ resolveWithObject: true });
    const raw = await sharp(template.data).raw().toBuffer({ resolveWithObject: true });
    const selected = new Uint8Array(raw.info.width * raw.info.height);
    ...
    const handRegions = [
        {
            ...
            right: 87,
            bottom: 299,
            polygon: [[52, 184], [59, 186], [70, 199], [77, 213], [81, 230], [80, 249], [77, 267], [72, 285], [65, 299], [52, 299]]
        },
        {
            left: 119,
            top: 194,
            right: 164,
            bottom: 299,
            polygon: [[164, 194], [153, 202], [143, 211], [135, 221], [129, 234], [124, 249], [127, 263], [133, 278], [145, 290], [158, 299], [164, 299]]
        }
    ];
    for (const region of handRegions) {
        for (let y = region.top; y < Math.min(region.bottom, raw.info.height); y++) {
            for (let x = region.left; x < Math.min(region.right, raw.info.width); x++) {
                if (!insidePolygon(x, y, region.polygon))
                    continue;
                const source = (y * raw.info.width + x) * raw.info.channels;
                const red = raw.data[source];
                const green = raw.data[source + 1];
                const blue = raw.data[source + 2];
                if (blue <= red * 1.15 || blue <= green * 1.05 || blue - red <= 12)
                    continue;
                selected[y * raw.info.width + x] = 1;
            }
        }
    }
    const outlined = selected.slice();
    for (const region of handRegions) {
        for (let y = region.top; y < Math.min(region.bottom, raw.info.height); y++) {
            for (let x = region.left; x < Math.min(region.right, raw.info.width); x++) {
                if (!insidePolygon(x, y, region.polygon))
                    continue;
                const pixel = y * raw.info.width + x;
                const source = pixel * raw.info.channels;
                if (selected[pixel] || raw.data[source] >= 75 || raw.data[source + 1] >= 85 || raw.data[source + 2] >= 100)
                    continue;
                let nextToHand = false;
                for (let offsetY = -1; offsetY <= 1 && !nextToHand; offsetY++) {
                    for (let offsetX = -1; offsetX <= 1; offsetX++) {
                        const neighbourX = x + offsetX;
                        const neighbourY = y + offsetY;
                        if (neighbourX < 0 || neighbourX >= raw.info.width || neighbourY < 0 || neighbourY >= raw.info.height)
                            continue;
                        if (selected[neighbourY * raw.info.width + neighbourX]) {
                            nextToHand = true;
                            break;
                        }
                    }
    ...
    const maskData = Buffer.alloc(raw.info.width * raw.info.height * 4);
    for (let pixel = 0; pixel < outlined.length; pixel++) {
        if (!outlined[pixel])
            continue;
        const target = pixel * 4;
        maskData[target] = 255;
        maskData[target + 1] = 255;
        maskData[target + 2] = 255;
        maskData[target + 3] = 255;
    }
    const handMask = await sharp(maskData, {
        raw: { width: raw.info.width, height: raw.info.height, channels: 4 }
    }).png().toBuffer();
    const hands = await sharp(template.data)
        .composite([{ input: handMask, blend: 'dest-in' }])
        .png()
        .toBuffer();
    return sharp(result).composite([{ input: hands }]).png().toBuffer();
}
