...
import { getStickerAuthor, getStickerPackName } from './stickerMetadata.js';
...
export async function sticker(isImage, url, _packname, _author) {
    try {
        ...
        return await new Sticker(buffer, {
            pack: getStickerPackName(),
            author: getStickerAuthor(),
...
export async function sticker5(img, url, packname, author, categories = [''], extra = {}) {
    const input = (url || img);
    return await new Sticker(input, {
        pack: getStickerPackName(packname),
        author: getStickerAuthor(author),
        type: StickerTypes.DEFAULT,
        categories,
        ...extra
    }).toBuffer();
