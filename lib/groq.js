...
function truncateVisionResponse(text, maxChars) {
    ...
    return `${(boundary > maxChars * 0.65 ? clipped.slice(0, boundary + 1) : clipped).trim()}…`;
}
...
function getConfig() {
    return {
        ...
        visionMaxTokens: config.groqVisionMaxTokens,
        visionMaxChars: config.groqVisionMaxChars
    };
}
...
export async function groqVision(buffer, mimetype = 'image/jpeg', prompt, options = {}) {
    const { visionModel, visionMaxTokens, visionMaxChars } = getConfig();
    ...
    const response = await groqRequest('/chat/completions', JSON.stringify({
        ...
        max_tokens: options.maxTokens || visionMaxTokens,
    ...
    if (!response)
        return null;
    const data = await response.json();
    const content = hideReasoning(data.choices?.[0]?.message?.content);
    return content ? truncateVisionResponse(content, options.maxChars || visionMaxChars) : null;
}
