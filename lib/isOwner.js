...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    ...
    if (sock && chatId && chatId.endsWith('@g.us')) {
        try {
            ...
            if (participant) {
                // WhatsApp may deliver consecutive messages from the same user
                // once as a LID and once as a phone JID. Compare every identity
                // exposed by group metadata instead of relying on the current
                // message's JID type.
                const participantIds = [participant.id, participant.lid, participant.phoneNumber]
                    .filter(Boolean)
                    .map(cleanJid);
                if (participantIds.includes(ownerNumberClean) ||
                    (ownerLidClean && participantIds.includes(ownerLidClean)) ||
                    await isSudo(participant.id) || await isSudo(participant.lid)) {
                    return true;
                }
            }
        }
...
function isOwnerOnly(senderId) {
    ...
    const ownerLidClean = cleanJid(config.ownerLid);
    const senderIdClean = cleanJid(senderId);
    return Boolean(senderIdClean === ownerNumberClean || (ownerLidClean && senderIdClean === ownerLidClean));
}
