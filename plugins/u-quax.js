    ...
    async handler(sock, message, args, context) {
        ...
        try {
            ...
            if (!hasMedia && !quotedMsg) {
                await sock.sendMessage(chatId, { text: `⚠️ ${t('p.quax.noMedia')}` }, { quoted: message });
                return;
            }
            ...
            if (!type) {
                await sock.sendMessage(chatId, { text: `⚠️ ${t('p.quax.unsupportedType')}` }, { quoted: message });
                return;
            }
            await sock.sendMessage(chatId, { text: t('p.quax.uploading') }, { quoted: message });
            ...
            await sock.sendMessage(chatId, {
                text: `✅ *${t('p.quax.success')}*\n\n🔗 ${result.url}`
            }, { quoted: message });
            fs.unlinkSync(tempPath);
        }
        catch (error) {
            console.error('Qu.ax Error:', error);
            await sock.sendMessage(chatId, { text: `❌ ${t('p.quax.errorLabel', { error: error.message })}` }, { quoted: message });
        }
    }
};
