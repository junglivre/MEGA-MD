    ...
    async handler(sock, message, args, context) {
        ...
        try {
            ...
            if (!hasMedia && !quotedMsg) {
                await sock.sendMessage(chatId, { text: `⚠️ ${t('p.xoat.noMedia')}` }, { quoted: message });
                return;
            }
            ...
            if (!type) {
                await sock.sendMessage(chatId, { text: `⚠️ ${t('p.xoat.unsupportedType')}` }, { quoted: message });
                return;
            }
            await sock.sendMessage(chatId, { text: t('p.xoat.uploading') }, { quoted: message });
            ...
            await sock.sendMessage(chatId, {
                text: `✅ *${t('p.xoat.uploadSuccess')}*\n\n🔗 ${result.url}`
            }, { quoted: message });
            fs.unlinkSync(tempPath);
        }
        catch (error) {
            console.error('X0.at Error:', error);
            await sock.sendMessage(chatId, { text: `❌ ${t('p.xoat.error', { message: error.message })}` }, { quoted: message });
        }
    }
};
