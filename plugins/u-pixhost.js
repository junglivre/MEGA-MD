    ...
    async handler(sock, message, args, context) {
        ...
        try {
            ...
            if (!hasMedia && !quotedMsg) {
                await sock.sendMessage(chatId, { text: `⚠️ ${t('p.pixhost.noMedia')}` }, { quoted: message });
                return;
            }
            ...
            if (!type) {
                await sock.sendMessage(chatId, { text: `⚠️ ${t('p.pixhost.unsupportedType')}` }, { quoted: message });
                return;
            }
            await sock.sendMessage(chatId, { text: t('p.pixhost.uploading') }, { quoted: message });
            ...
            await sock.sendMessage(chatId, {
                text: `✅ *${t('p.pixhost.success')}*\n\n🔗 ${result.url}`
            }, { quoted: message });
            fs.unlinkSync(tempPath);
        }
        catch (error) {
            console.error('Pixhost Error:', error);
            await sock.sendMessage(chatId, { text: `❌ ${t('p.pixhost.errorLabel', { error: error.message })}` }, { quoted: message });
        }
    }
};
